...
| `admin_grant` (`AdminGrant`: `kind` configured / break_glass, `session_id`) | `IsAdmin` filter, reused by `AdminOnlyMiddleware` (admin router only) |
...
- `/emergency_admin` password step ([app/handlers/user/emergency_admin.py](app/handlers/user/emergency_admin.py)) — deletes the password message, then inside a per-user `keyed_lock` runs `EmergencyAdminAuthService.authenticate` and commits (attempt row, session row, user-row lock released) before answering; a denial is `admin.access_denied`, a success `admin.emergency_granted` followed by `show_admin_menu`. Silent when `settings.emergency_admin_enabled` is False. Its texts live under `admin.` so the customer-journey coverage test skips them.
- 🪪 Loyalty credit confirm ([app/handlers/admin/loyalty.py](app/handlers/admin/loyalty.py)) — commits inside `confirm_once` right after `credit_stamps`, before the operator is answered, so the account lock never spans a Telegram call; a refusal rolls back first.
...
- Manual stamp credits go through `AdminLoyaltyService.credit_stamps` in [app/services/admin/loyalty.py](app/services/admin/loyalty.py) (policy from `LOYALTY_ADMIN_MAX_STAMP_ADJUSTMENT`; `AdminService(session, settings=…)` exposes it as `loyalty_admin`). It validates everything first (whole number 1..max, non-blank reason ≤ 255, a UUID operation id, actor ≠ target, registered customer), locks the account, and either returns the credit already booked under that operation id (`created=False`) or calls `LoyaltyService.adjust` and writes the `loyalty_stamp_adjustments` author row (operator, `AdminAccessKind`, session id, operation id — unique per ledger row and per operation id). It issues no reward (the customer claims on the card, as after a purchase), touches no purchase count, imports nothing that can send a message; `loyalty_health` reports adjustments without an author under `audit` (tests and migrations may write them through `LoyaltyService.adjust`; production should show 0) and flags author rows that disagree with their ledger row under `integrity`. The target is named through `AdminUserService.resolve_customer` ([app/services/admin/users.py](app/services/admin/users.py)): digits are a Telegram id (exact row or `CustomerNotFoundError`), `@name` / `t.me/name` a handle matched case-insensitively against `users.username`, which is not unique and only as fresh as the customer's last message — no match is `CustomerNotFoundError`, several are `AmbiguousCustomerError`, anything else `MalformedCustomerIdentifierError` (all `CustomerLookupError`, a `ValueError`). It returns a `CustomerIdentity` (canonical `telegram_id`, stored username and name, `label` for the confirm screen — escape it with `e()`); a confirm step re-checks with `resolve_telegram_id`, never with the handle, and nothing a client sends here is authorization. The literal `.adjust(` stays service-only and `.credit_stamps(` has one caller, the 🪪 Loyalty wizard in [app/handlers/admin/loyalty.py](app/handlers/admin/loyalty.py) (`tests/test_stamp_card.py`): `/admin_adjust_stamps` or the menu button → customer → amount → a confirmation whose button carries only a fresh UUID operation id kept in FSM data with the target's Telegram id and the amount; the tap runs inside `confirm_once` + `keyed_lock`, rejects any id but the one in the data (`error.invalid_callback`), re-resolves the customer by id, books through `credit_stamps` with `AdjustmentActor.from_grant(operator.id, admin_grant)`, commits before answering, and a stale button (no state, after a restart, another operator's) is answered by a last `stale_credit_button` handler since the root fallback ignores `admin:` callbacks. `AdjustStampsStates` is in `ADMIN_WIZARD_STATES`; `admin.menu_loyalty` is in the wizard guard's menu keys. `tests/test_admin_stamp_adjustment.py` pins all of this.
- SQLite ignores `FOR UPDATE`. Concurrency is only proven by the opt-in PostgreSQL suites — `tests/test_loyalty_postgres.py` (per operation), `tests/test_loyalty_journeys_postgres.py` (whole updates), `tests/test_loyalty_concurrency.py` (combinations, and no lock held across Telegram) — which need `VSHOP_TEST_POSTGRES_URL` (database name must end in `_test` and be empty — each creates and drops the schema); a release run adds `VSHOP_TEST_NO_SKIPS=1`, which fails the session on any skipped test (`tests/conftest.py`). Correctness rests on READ COMMITTED plus one lock order (`keyed_lock` → cart → order → the customer's own account → referral → the referrer's account, up the chain → grant/reward → attribution advisory lock; a payout's referral row always comes after the referred customer's account, which the completion's stamp award already holds; the per-operation table is in `docs/architecture.md`); nothing retries, because every operation is idempotent; `tests/test_loyalty_scenarios.py` checks on every run that each redemption path takes the account lock before its first write (`tests/test_loyalty_guards.py` does the same for a spin and a referral attribution), alongside the owner's other end-to-end scenarios. `loyalty_health` in `app/verify_deployment.py` cross-checks the tables after a deploy.

### Admin access sessions (break-glass, persistence only)

`admin_access_sessions` (`AdminAccessSession` in [app/models/admin_access.py](app/models/admin_access.py), `AdminAccessService` in [app/services/admin_access.py](app/services/admin_access.py)) holds temporary admin grants: a `users` FK, `auth_method` (`break_glass`), `expires_at`, `revoked_at`. Active means unrevoked and `expires_at > now`, with `now` from the service's injectable clock — both timestamps are written from that clock, never `func.now()`, because a CHECK compares them. `open_break_glass` bounds the TTL (`MIN_SESSION_TTL`..`MAX_SESSION_TTL`) and revokes the user's earlier active session; revocation sets a timestamp and never deletes; the model refuses changing `user_id`, `auth_method`, `expires_at` or a set `revoked_at`. It stores no credential and adds nobody to `ADMIN_IDS`. Authorization consults it: `resolve_admin_grant` in [app/security/admin.py](app/security/admin.py) is the one decision — a configured `ADMIN_IDS` id is granted from settings alone (no query, exactly as before), anyone else only by an active session read from the database on every update, and no database session means *no* (fail closed). With `EMERGENCY_ADMIN_PASSWORD_HASH` unset no session counts (the kill switch, no query either), and `lifecycle.reconcile_emergency_sessions` revokes every active session at startup while the feature is off. `AdminOnlyMiddleware` logs every update served under a break-glass grant with its session id. `IsAdmin` calls it once and returns `{"admin_grant": grant}`, which aiogram merges into the handler data; `AdminOnlyMiddleware` reuses that grant and re-resolves only when it is missing or names another user; `IsNotAdmin` (the user router's `/admin` denial) negates it. No handler checks emergency access itself — `tests/test_admin_authorization.py` scans `app/handlers/` for any import of the access modules. `is_admin_user` / `is_admin_id` stay configured-only helpers (new-order alerts, tests), not authorization gates. A break-glass holder gets the very same panel and handlers; a stale admin button after expiry or revocation is dropped silently like a stranger's. Migration `d7a3f9c2e8b1`; the downgrade refuses while rows exist (`alembic -x allow_admin_access_data_loss=true downgrade …`). `tests/test_admin_access_sessions.py` pins all of this.

Authentication is `EmergencyAdminAuthService` in [app/services/emergency_admin.py](app/services/emergency_admin.py) (`EmergencyAccessPolicy.from_settings`; the four `EMERGENCY_ADMIN_*` settings). `authenticate(user_id, password)` returns an `AuthenticationResult`, never raises for anything a user can cause, and the handler must answer every denial with the same text — `DenialReason` is for the log and the audit only. Order inside: feature off → denied, nothing recorded; user row locked (`UserRepository.get_for_update`, the only user-row lock in the code, never held across Telegram); locked out → recorded `locked_out`, not checked; empty → recorded `failed`, not hashed; wrong → `failed`; right → `succeeded` + `AdminAccessService.open_break_glass` (supersedes the earlier session). The hash is a `SecretStr`; verification is `verify_password` in [app/utils/passwords.py](app/utils/passwords.py) (scrypt, `hmac.compare_digest`, run through `asyncio.to_thread`) — that module lives in `app/utils/` because `app/security/__init__.py` imports `app.config`, which imports it; the interactive hash tool is the top-level `python -m app.hash_emergency_password`, kept out of `app/utils/` because `tests/test_localization_audit.py` forbids prose literals there. Never log or format the password or the hash; `tests/test_emergency_admin_auth.py` scans the service's logging calls for it. Attempts live in `admin_access_attempts` (migration `e8b2c4d6f1a3`); a successful login resets the failure count.

### Router composition

- [app/handlers/user/__init__.py](app/handlers/user/__init__.py) — `start` first (so `/start` wins), then `emergency_admin` (`/emergency_admin` and its password step must beat every free-text handler), then catalog/cart, stamp_card, roulette and invite (ahead of checkout, so their menu buttons win over its free-text steps), checkout, info, and `admin_guard` last (the non-admin `/admin` denial must live outside the admin router's filters).
- [app/handlers/admin/__init__.py](app/handlers/admin/__init__.py) — double-gated: router-level `IsAdmin` filters *and* `AdminOnlyMiddleware` (drops unauthorized updates silently); both decide through `resolve_admin_grant` (`ADMIN_IDS` or an active break-glass session). `wizard_guard` is included before the section routers so menu taps mid-FSM are intercepted; `panel` is last.
...
- Every user-facing string goes through `i18n.t(key, **kwargs)`. Missing keys fall back to English, then return the key itself. `t`, `plural` and `translate` take their key (and count or language) positional-only, so a placeholder may have any name — `{language}` included; `tests/test_localization.py` renders every template in every language with its placeholders filled.
...
- `keyed_lock(key)` ([app/utils/concurrency.py](app/utils/concurrency.py)) — process-local async lock (checkout uses it directly; admin wizards get it through `confirm_once`). Its registry resets when the running event loop changes: the bot has one loop for life, but pytest gives every test its own, and a lock contended in one test must not be handed to the next (`tests/test_keyed_lock_loops.py`).
...
- [tests/test_migrations.py](tests/test_migrations.py) — static checks on `alembic/versions/`: `upgrade()` — and any module function it calls — may not `drop_table`/`drop_column`/`drop_constraint` (on `op`, `batch_op` or anything else) or run destructive raw SQL, and its raw SQL must be a literal the test can read, directly, through `sa.text` or a module constant (expand/contract: removal goes in a separate later migration). Index drops must be whitelisted in `ALLOWED_INDEX_DROPS` with a reason. `downgrade()` may not be empty, the chain must stay linear with one head, file names must start with the revision id, and every model table must be created by some migration (tests use `create_all`, so a missing migration only fails on deploy). A column, index or constraint missing from the migrations is caught by `alembic check` in `tests/test_loyalty_activation_postgres.py`.

## Deployment safety

- Compose pins the project name (`vshop`). The Postgres volume is `external` and named by the required `POSTGRES_VOLUME_NAME` (no default), so Compose never creates or deletes it: a missing or wrong name stops `docker compose up` instead of starting the bot on a new, empty database — the catalog-loss incident. A new deployment runs `docker volume create vshop_pgdata` first. `tests/test_documentation.py` pins both; don't change either without updating `docs/deployment.md`.
...
- Parse mode is HTML globally (`DefaultBotProperties`). Interpolate any user- or DB-supplied value through `e()` from [app/utils/html.py](app/utils/html.py) — admin-typed text too (product previews, the order search). Telegram refuses a message it cannot parse, so a missed `e()` is a screen nobody sees; `tests/test_html_escaping.py` refuses such HTML the way Telegram does.
- Admin text steps refuse what the column cannot hold: the product wizards check `text_limit` (`app/models/product.py`), categories and brands `max_len=255`. SQLite stores an overlong `VARCHAR`; PostgreSQL refuses the write.
...
- Extended docs live in [docs/](docs/) (installation, configuration, architecture, loyalty, roulette, referrals, database schema, deployment, security, testing, admin guide), plus `CHANGELOG.md` at the root, and are kept current — update them alongside structural changes (several facts are enforced by the tests above). Mermaid diagrams in them must render. Never add a README inside docs/: `tests/test_documentation.py` keys the documents by file name, so it would collide with the root `README.md`.
